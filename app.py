...
# Configuration constants - TETAP SAMA
BASE_UNITS = 16
BASE_DROPOUT = 0.01
BASE_BATCH = 16
BASE_EPOCHS = 10
...
def main():
    ...
    if df is not None and not df.empty:
        ...
        with training_col1:
            ...
            # Hanya tombol train saja - OTOMATIS
            st.info("**Automatic Configuration:**")
            st.write("• Lookback Days: 1")
            st.write("• Train-Test Split: 80:20") 
            st.write("• PSO Optimization: Enabled")
            ...
            if st.button("🚀 Train Model", type="primary", use_container_width=True):
                with st.spinner("Training model... This may take several minutes"):
                    try:
                        ...
                        n = len(df)
                        n_train = int(n * 0.8)  # OTOMATIS 80:20
                        n_test = n - n_train
                        ...
                        # Create sequences dengan window=1 (OTOMATIS)
                        lookback_days = 1
                        ...
                        # PSO Optimization - OTOMATIS ENABLED
        ...
        with training_col2:
            if st.session_state.get('model_trained', False):
                ...
                with col_metric1:
                    st.metric(
                        "Baseline MAPE", 
                        f"{baseline_results['mape']:.4f}%",
                        delta=f"Baseline LSTM"
                    )
                ...
                if 'final_results' in st.session_state:
                    ...
                    with col_metric2:
                        st.metric(
                            "PSO-LSTM MAPE", 
                            f"{final_results['mape']:.4f}%",
                            delta="Optimized Model",
                            delta_color="normal"
                        )
                    ...
                    st.success(f"🎯 **Improvement: MAPE decreased by {impr_mape:.2f}%**")
